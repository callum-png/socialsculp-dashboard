...
const step1Schema = z.object({
  ...
  brandIndex: z.string().min(1, 'Please select a brand'),
})
...
function inputCls(hasError?: boolean) {
  return cn(
    ...
    hasError ? 'border-[#FF4747] focus:border-[#FF4747]' : 'border-[#222222] focus:border-[#C9FF47]'
  )
}
...
export default function NewCampaignPage() {
  ...
  const form1 = useForm<Step1Data>({
    ...
    defaultValues: { name: '', description: '', brandIndex: '' },
  })
  ...
  function handleCreate() {
    ...
    console.log('Campaign created:', final)
    router.push('/admin/campaigns')
  }

  const brandName =
    formData.brandIndex !== undefined
      ? MOCK_BRANDS[Number(formData.brandIndex)]?.companyName
      : '—'
              ...
              <div className="flex items-center gap-2">
                <div
                  className={cn(
                    ...
                    i < step
                      ? 'bg-[#C9FF47] border-[#C9FF47] text-[#090909]'
                      : i === step
                        ? 'bg-[#1A1A1A] border-[#C9FF47] text-[#C9FF47]'
          ...
          <form onSubmit={form1.handleSubmit(handleStep1)} className="space-y-5">
            <div className="bg-[#111111] border border-[#222222] p-6 space-y-5">
              ...
              <div>
                <Label>Brand</Label>
                <select
                  {...form1.register('brandIndex')}
                  className={inputCls(!!form1.formState.errors.brandIndex)}
                >
                  <option value="">Select a brand…</option>
                  {MOCK_BRANDS.map((brand, i) => (
                    <option key={brand.companyName} value={i}>
                      {brand.companyName}
                    </option>
                  ))}
                </select>
                <FieldError message={form1.formState.errors.brandIndex?.message} />
              </div>
            </div>
            ...
            <div className="flex justify-end">
              <button
                type="submit"
                className="inline-flex items-center gap-2 px-5 py-2.5 bg-[#C9FF47] text-[#090909] text-xs font-syne font-bold uppercase tracking-widest hover:bg-[#b8ee36] transition-colors"
              >
                      ...
                      <div
                        className={cn(
                          ...
                          form2.watch('platform') === p
                            ? 'border-[#C9FF47] text-[#C9FF47] bg-[#0D1F00]'
          ...
          <div className="space-y-5">
            <div className="bg-[#111111] border border-[#222222] p-6 space-y-4">
              ...
              <div className="space-y-3">
                {[
                  ...
                  { label: 'Budget', value: formData.totalBudget ? `$${formData.totalBudget.toLocaleString()}` : '—' },
                  ...
                  { label: 'Target Reach', value: formData.targetReach ? formData.targetReach.toLocaleString() : '—' },
                ].map((row) => (
            ...
            <div className="flex justify-between">
              ...
              <button
                ...
                className="inline-flex items-center gap-2 px-6 py-2.5 bg-[#C9FF47] text-[#090909] text-xs font-syne font-bold uppercase tracking-widest hover:bg-[#b8ee36] transition-colors"
              >
