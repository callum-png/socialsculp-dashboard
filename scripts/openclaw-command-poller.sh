...
DASHBOARD_URL="${CLAUDE_DASHBOARD_URL:-https://socialsculp-dashboard.vercel.app}"
REPORTER_SECRET="${CLAUDE_REPORTER_SECRET}"
CLAUDE_HOME="${CLAUDE_HOME:-$HOME/.claude}"

if [ -z "$REPORTER_SECRET" ]; then
  echo "[cmd-poller] CLAUDE_REPORTER_SECRET not set" >&2
  exit 1
fi

# Whitelist (redundant safety — dashboard also validates)
# Paths are relative to $CLAUDE_HOME for portability
ALLOWED_PREFIXES=(
  "claude"
  "launchctl list ai.claude"
  "cat ${CLAUDE_HOME}/"
  "ls ${CLAUDE_HOME}/"
  "tail ${CLAUDE_HOME}/"
  "uptime"
...
RESPONSE=$(curl -s -w "\n%{http_code}" \
  "${DASHBOARD_URL}/api/claude-agent/commands/pending" \
...
echo "$COMMANDS" | while IFS= read -r CMD_JSON; do
  ...
  if ! is_allowed "$CMD_TEXT"; then
    # Post back as failed
    curl -s -X POST "${DASHBOARD_URL}/api/claude-agent/commands/result" \
